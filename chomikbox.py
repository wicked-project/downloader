...
class Chomik(object):
    ...
    def __access_node(self, directories, folders_dom):
        if len(directories) == 0:
            return folders_dom
        folders_dom = folders_dom['folders']['FolderInfo']
        #TODO - utf
        if "name" in folders_dom and folders_dom['name'] == unescape_name(to_unicode(directories[0])):
            return self.__access_node(directories[1:], folders_dom)
        if type(folders_dom) == type([]):
            for inner_dict in folders_dom:
                if "name" in inner_dict and inner_dict['name'] == unescape_name(to_unicode(directories[0])):
